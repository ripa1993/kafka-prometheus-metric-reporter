...
class YammerExports(registry: MetricsRegistry) extends Collector with Collector.Describable with LazyLogging {
  ...
  private[kpr] def fromCounter(metricName: MetricName, counter: Counter) = {
    val sample = sampleBuilder.createSample(makeMetricName(metricName), "", makeLabelNames(metricName).asJava, makeLabelValues(metricName).asJava, counter.count.toDouble)
  ...
  private[kpr] def fromGauge(metricName: MetricName, gauge: Gauge[_]): Collector.MetricFamilySamples = {
    ...
    obj match {
      ...
      case _ =>
        logger.trace(String.format("Invalid type for Gauge %s: %s", Collector.sanitizeMetricName(metricName.getName), if (obj == null) "null"
        else obj.getClass.getName))
        return null
    }
    val sample = sampleBuilder.createSample(makeMetricName(metricName), "", makeLabelNames(metricName).asJava, makeLabelValues(metricName).asJava, value)
  ...
  private[kpr] def fromSnapshotAndCount(metricName: MetricName, snapshot: Snapshot, count: Long, factor: Double, helpMessage: String) = {
    val samples = util.Arrays.asList(sampleBuilder.createSample(makeMetricName(metricName), "", util.Arrays.asList("quantile"), util.Arrays.asList("0.5"), snapshot.getMedian * factor), sampleBuilder.createSample(makeMetricName(metricName), "", (List("quantile") ++ makeLabelNames(metricName)).asJava, (List("0.75")  ++  makeLabelValues(metricName)).asJava, snapshot.get75thPercentile * factor), sampleBuilder.createSample(makeMetricName(metricName), "", (List("quantile") ++ makeLabelNames(metricName)).asJava, (List("0.95")  ++  makeLabelValues(metricName)).asJava, snapshot.get95thPercentile * factor), sampleBuilder.createSample(makeMetricName(metricName), "", (List("quantile") ++ makeLabelNames(metricName)).asJava, (List("0.98")  ++  makeLabelValues(metricName)).asJava, snapshot.get98thPercentile * factor), sampleBuilder.createSample(makeMetricName(metricName), "", (List("quantile") ++ makeLabelNames(metricName)).asJava, (List("0.99")  ++  makeLabelValues(metricName)).asJava, snapshot.get99thPercentile * factor), sampleBuilder.createSample(makeMetricName(metricName), "", (List("quantile") ++ makeLabelNames(metricName)).asJava, (List("0.999")  ++  makeLabelValues(metricName)).asJava, snapshot.get999thPercentile * factor), sampleBuilder.createSample(makeMetricName(metricName), "_count", makeLabelNames(metricName).asJava, makeLabelValues(metricName).asJava, count))
  ...
  /**
    ...
    */
  private[kpr] def fromHistogram(metricName: MetricName, histogram: Histogram) = fromSnapshotAndCount(metricName, histogram.getSnapshot, histogram.count(), 1.0, getHelpMessage(metricName.getName, histogram))

  /**
    * Export Dropwizard Timer as a histogram. Use TIME_UNIT as time unit.
    */
  private[kpr] def fromTimer(metricName: MetricName, timer: Timer) = fromSnapshotAndCount(metricName, timer.getSnapshot, timer.count(), 1.0D / TimeUnit.SECONDS.toNanos(1L), getHelpMessage(metricName.getName, timer))

  /**
    * Export a Meter as as prometheus COUNTER.
    */
  private[kpr] def fromMeter(metricName: MetricName, meter: Meter) = {
    val sample = sampleBuilder.createSample(makeMetricName(metricName), "_total", makeLabelNames(metricName).asJava, makeLabelValues(metricName).asJava, meter.count())
  ...
  private def getHelpMessage(metricName: String, metric: Metric) = String.format("Generated from Yammer metric import (metric=%s, type=%s)", metricName, metric.getClass.getName)
